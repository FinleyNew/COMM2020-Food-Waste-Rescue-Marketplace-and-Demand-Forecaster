...
# Endpoint for consumers to create issue reports
...
# Endpoint for sellers to repond to issue reports
@router.patch("/{issue_id}/{response}", response_model=IssueReportPublic)
def respond_to_issue_report(issue_id: int, response: str, current_seller: SellerDep, db: SessionDep):
...
# Endpoint for getting the current consumers reports for a specific bundle
...
# Endpoint for getting all issue reports
...
# Endpoint for setting an issue report resolved
...
# Endpoint for deleting a specific issue report
# Can only be used by admins
